...
(setf the-input-list 
      (let* ((x (split-string the-input-string :delimiter #(#\newline)))
             ...
             (z (mapcar (lambda (a) (list (split-string (car a) :delimiter #(#\space)) (split-string (cadr a) :delimiter #(#\space)))) y)))
        z))
...
(defun a (b)
    (let ((c 0))
        (mapcar 
            (lambda (a) (let ((d (length a))) (if (or (= d 2) (= d 4) (= d 3) (= d 7)) (setf c (1+ c)) nil)))
            b)
        c))
...
(defun c (d)
    (let (  (e (car d))
            ...
            (g (make-hash-table))
            (h (make-hash-table)))
        (setf (gethash #\a g) 0 (gethash #\b g) 0 (gethash #\c g) 0 (gethash #\d g) 0 (gethash #\e g) 0 (gethash #\f g) 0 (gethash #\g g) 0)
        (mapcar 
            (lambda (a) (loop for i from 0 to (1- (length a)) do
        ...
        (parse-integer 
            (with-output-to-string (*standard-output*) 
                (mapcar 
                    (lambda (a) (let ((j (make-hash-table))) 
                        (loop for i from 0 below (length a) do
                            (setf (gethash (gethash (char a i) h) j) t))
                        (let ((the-list (loop for i from 1 to 7 collect (gethash i j))))
                            (princ (f the-list))))) 
                    f)))))

(defun d (e f)
    (let (g)
        (maphash (lambda (k v) (if (= f v) (setf g k) nil)) e)
        g))

(defun e (f g)
    (let (h)
        (loop for i from 0 below (length g) do
            (if (and (null h) (null (gethash (char g i) f)))
                (setf h (char g i))
                nil))
        h))

(defun f (g)
    (cond   ((equal g '(t   t   t   nil t   t   t   )) 0)
            ((equal g '(nil nil t   nil nil t   nil )) 1)
            ((equal g '(t   nil t   t   t   nil t   )) 2)
            ((equal g '(t   nil t   t   nil t   t   )) 3)
            ((equal g '(nil t   t   t   nil t   nil )) 4)
            ((equal g '(t   t   nil t   nil t   t   )) 5)
            ((equal g '(t   t   nil t   t   t   t   )) 6)
            ((equal g '(t   nil t   nil nil t   nil )) 7)
